...
export default function AssignProtocolModal({ open, onClose, patientId, templates, activeProtocolTitle }) {
    ...
    return (
        <Modal open={open} onClose={close} title="Assigner un protocole" maxWidth={520}>
            {activeProtocolTitle && (
                <div className="mb-5 rounded-xl border border-terracotta/30 bg-terracotta/10 px-4 py-3 text-sm text-forest">
                    Un protocole actif existe déjà (« {activeProtocolTitle} »). Confirmer ci-dessous archivera automatiquement ce
                    protocole.
                </div>
            )}
            ...
            <form onSubmit={submit} className="flex flex-col gap-5">
                <div>
                    <p className="mb-2 text-sm font-semibold text-forest">Point de départ</p>
                    <div className="flex flex-col gap-2">
                        ...
                        >
                            Protocole vierge
                            <span className="block text-xs text-forest/50">Partir de zéro, ajouter les items ensuite</span>
                        </button>
                ...
                >
                    {activeProtocolTitle ? 'Archiver et assigner' : 'Assigner le protocole'}
                </button>
            </form>
        </Modal>
    );
